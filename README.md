...
**[[Cleaned Dataset — Google Drive]](YOUR_DRIVE_LINK_HERE)** · **[[Technical Report]](YOUR_REPORT_DRIVE_LINK_HERE)** · **[[HNG Internship Certificate]](YOUR_CERTIFICATE_LINK_HERE)**
...
The dataset had zero duplicate `product_id` values, which looked clean at first. But 4 rows had identical `title` values pointing to different product IDs — the same product listed twice under different identifiers. Removing the title rather than ID was the right call. Removing on ID alone would have left the problem in place.
...
This was built as HNG14 Internship Stage 1 Task, April 2026. The constraint of working entirely in Excel — no Python, no SQL — forced a disciplined approach to what formulas alone can accomplish when the logic is sound.
